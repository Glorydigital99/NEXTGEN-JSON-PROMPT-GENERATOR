...
const MASTER_TRIAL_PROTOCOL = {
  ...
  "branding_footer": { "credit_text": "© NEXTGEN","disclaimer": "Visual generated by NEXTGEN."},
...
const MASTER_ELITE_PROTOCOL = {
   ...
   "reusable": true,
    ...
    "font_profiles": {"headline_font": "Custom Display Hybrid — organic curves, bold strokes, cultural-neutral, cinematic weight",
    "subheadline_font": "Expressive Neo-Humanist — semi-rounded, wide aperture, high legibility",
    "body_font": "Experimental Readable Grotesk — soft geometry, thick baseline, anti-noise",
    "caption_font": "Micro-Readable Display — optimized for small size & mobile zoom"}},
    ...
    "content_purity_policy": {
    "no_style_mentions": true,
    "forbidden_keywords": ["visual", "3D", "cinematic", "editorial", "render", "8K"],
    "focus_only": "pure_product_features_and_benefits" } } } };
...
const INITIAL_ELITE = [
  ...
  { id: 'sentient_system_aesthetic', label: 'Sentient System Aesthetic', desc: 'Cocok untuk: AI visioner, sistem sadar, poster “beyond human”', type: 'cpu' }
];
...
const App = () => {
  ...
  const fetchPremiumReport = async () => {
    ...
    try {
      ...
      for (const tokenDoc of querySnapshot.docs) {
        ...
        reportText += `👤 ${uSnap.exists() ? uSnap.data().name : "Unknown"}\n🔑 ${info.premium_token}\n--------------------\n`;
      }
  ...
  const fetchUserList = async () => {
    ...
    try {
      ...
      querySnapshot.forEach((doc) => {
        const data = doc.data();
        userText += `${data.isPremium ? "💎" : "🆓"} ${data.name}\n📧 ${data.email}\nAcc: ${data.isApproved ? "✅" : "❌"} | Gen: ${data.dailyCount}/10\n--------------------\n`;
      });
      alert(userText);
  ...
  const generateBulkTokens = async () => {
    ...
    try {
      ...
      alert("✅ 50 Token baru ditambahkan.");
...
// --- [NEURAL ADMIN COMMAND CENTER] ---
  const runAdminCommand = async () => {
    ...
    if (command === "REPORT") {
      ...
      let txt = "📊 SALES REPORT\n\n";
      for (const d of snap.docs) { txt += `🔑 ${d.data().premium_token} (USED)\n---\n`; }
    ...
    // Cek Stok Token (50 Biji + Auto Copy)
      ...
      alert("🔑 READY TO SELL (50 STOK COPIED):\n\n" + keys.join("\n"));
    } 
    ...
    // Cetak Token Baru
      ...
      alert("✅ 50 Token berhasil masuk brankas.");
    } 
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const promptText = `
        SYSTEM TASK: Generate a PURE JSON prompt based on MASTER_PROTOCOL.
        
        PRODUCT DATA:
        - Identity: "${formData.title}"
        - Context: "${formData.description}"
        - Chosen Style: "${selectedStyle.label}"
        
        STRICT OUTPUT RULES:
        1. NO STYLE LEAKAGE: The fields "description", "content_points", and "impact_text" MUST NOT contain words related to visual styles (e.g., NO "3D", NO "cinematic", NO "editorial", NO "render", NO "8K").
        2. Purity Rule: Focus 100% on the product features and benefits in the narrative fields.
        3. Visual style instructions must stay ONLY in the metadata or visual_logic fields.
        
        MASTER SETTINGS: ${JSON.stringify(activeMaster)}
      `;
      ...
      <Shield size={16} />
            ...
            <div className="w-full grid grid-cols-1 lg:grid-cols-12 gap-6 items-start px-2 md:px-10 justify-items-stretch">
              ...
              <div className="lg:col-span-3 w-full">
                ...
                <div className="space-y-3 max-h-[162.5] overflow-y-auto pr-2 custom-scrollbar">
